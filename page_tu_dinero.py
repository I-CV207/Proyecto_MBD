...
# ─────────────────────────────────────────────────────────────
    ...
    for i, row in df_metas.iterrows():
        ...
        colores = ["#8ECA62", "#C3D755", "#61ACAB"]  # verde, amarillo, rojo (puedes cambiarlos a tu gusto)
        fondo = colores[i]  # usamos el índice directamente porque hay máximo 3 metas
        ...
        with cols[i % 3]:
            ...
            st.markdown(
                ...
                <div style="
                    ...
                    margin-bottom: 20px;
                ">
                    <h4 style="margin: 0 0 10px 0; font-weight: 600;">{row['nombre_meta']}</h4>
                    <p style="font-size: 30px; margin: 0;"><strong>${row['monto_actual']:,.0f}</strong></p>
                    <p style="margin: 0 0 15px 0;">de ${row['monto_objetivo']:,.0f} ahorrados</p>
                """,
                unsafe_allow_html=True
            )
            # Barra de progreso
            st.markdown(f"""
            <div style="background-color: {fondo}; border-radius: 15px; height: 15px; width: 100%; margin-bottom: 10px;margin-top: -40px">
            <div style="background-color: #e0e0e0; border-radius: 8px; height: 16px; width: 80%; margin-bottom: 10px;margin-left: 40px">
            <div style="
                width: {progreso:.1f}%;
            ...
            st.markdown(
                ...
                ">
                    <strong>Progreso:</strong> {progreso:.1f}% &nbsp;|&nbsp;      
                          Te faltan <strong>${restante:,.0f}</strong> y {row['plazo_meses']} meses para lograrlo
                </div>
                """,
                unsafe_allow_html=True
            )
            # Formularios de control de metas
    ...
    st.markdown(
        ...
        <div style="
            ...
            background: linear-gradient(to right, #2e7d67, #4db6ac);
            border-radius: 20px;
            padding: 20px 30px;
            margin-top: 40px;
            color: white;
        ...
        ">
            <img src="https://raw.githubusercontent.com/I-CV207/Proyecto_MBD/main/Billie_inversor.png" alt="Aprende a invertir" style="height: 80px; border-radius: 12px; margin-right: 20px;">
            <div style="flex: 1;">
                <p style="margin: 0; font-size: 18px;">
                    <strong>¡Maximiza tus finanzas</strong>, potencia el crecimiento de tus<br>
                    ahorros <strong>aprendiendo a invertir!</strong>
                </p>
            </div>
            <a href="/page_perfil_ahorro_V2" style="text-decoration: none;">
                <button style="
