...
st.markdown("""
    <style>
        ...
        /* Cambia color de texto de los inputs */
...
modelo_url = "https://drive.google.com/uc?export=download&id=1W0jRvN64cSlbKJG8ESlOdHvAZ9up-8dL"
columnas_url = "https://drive.google.com/uc?export=download&id=1uB-WGLPextKOBlXWaLREpEcJChfU9NSQ"

os.makedirs("modelos", exist_ok=True)
if not os.path.exists(modelo_path):
    response = requests.get(modelo_url, allow_redirects=True)
...
if not os.path.exists(columnas_path):
    response = requests.get(columnas_url, allow_redirects=True)
    if response.status_code == 200:
        with open(columnas_path, "wb") as f:
            f.write(response.content)
    else:
        st.error("❌ Error al descargar las columnas del modelo.")
        st.stop()
...
with st.expander("Descrubre o modifica tu perfil financiero", expanded=st.session_state.get("modificar_perfil", False)):
    ...
    inversion = st.selectbox("Si inviertes 10,000 y al siguiente mes disminuye a 8,500, ¿Qué harías?", [
        ...
        "Me tranquilizo, lo dejo minimo un año",
        "Compro más acciones, es una buena oportunidad de compra"])
    ...
    def clasificar_perfil_riesgo(inversion, comodidad):
        if inversion == "Lo retiro de inmediato": return "Conservador"
